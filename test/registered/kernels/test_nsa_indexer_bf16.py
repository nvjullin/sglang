...
class TestBF16HadamardGuard(_NSAIndexerBF16Base):
    ...
    def test_k_cache_stored_with_hadamard_in_bf16_prefill(self, mock_dg):
        """In BF16 prefill mode, the key passed to _store_index_k_cache must equal
        rotate_activation(raw_key), where raw_key is the output of _get_q_k_bf16.
        Verified by exploiting the fact that Hadamard is its own inverse:
        rotate_activation(rotate_activation(x)) == x, so
        rotate_activation(stored_key) ≈ raw_key."""
        mock_dg.get_num_sms.return_value = 132
        self._set_server_args(bf16=True)
        ...
        # Small-scale inputs to avoid NaN from random weight init
        torch.manual_seed(42)
        total = batch_size * seq_len
        x = (
            torch.randn(
        ...
        captured = {}
        original_get_q_k = Indexer._get_q_k_bf16

        def spy_get_q_k(self_inner, *args, **kwargs):
            q, k = original_get_q_k(self_inner, *args, **kwargs)
            captured["raw_k"] = k.clone()
            return q, k

        def spy_store(self_inner, forward_batch, layer_id, key, **kwargs):
            captured["stored_k"] = key.clone()
        ...
        with patch(
            ...
            mock_aq.side_effect = lambda x_in, *a, **kw: (
            ...
            # Small weights to avoid NaN propagation through the linear layers
            with torch.no_grad():
                indexer.wk.weight.normal_(0, 0.01)
                indexer.wq_b.weight.normal_(0, 0.01)
            ...
            with patch.object(Indexer, "_get_q_k_bf16", spy_get_q_k), patch.object(
                Indexer, "_store_index_k_cache", spy_store
            ), patch.object(backend, "get_indexer_metadata", return_value=metadata):
                indexer(
                    x=x,
                    q_lora=q_lora,
                    positions=positions,
        ...
        self.assertIn(
            "raw_k", captured, "_get_q_k_bf16 was not called / raw_k not captured"
        )
        self.assertIn(
            "stored_k",
            captured,
            "_store_index_k_cache was not called / stored_k not captured",
        )

        raw_k = captured["raw_k"]
        stored_k = captured["stored_k"]

        # Hadamard is its own inverse: rotate_activation(rotate_activation(x)) == x
        # So if stored_k = rotate_activation(raw_k), then rotate_activation(stored_k) ≈ raw_k.
        # Import locally — the top-level import was removed to satisfy ruff F821 (the name
        # also appears as a patch-target string, confusing ruff's usage tracking).
        from sglang.srt.layers.attention.nsa.nsa_indexer import (
            rotate_activation as _rotate_activation,
        )

        reconstructed = _rotate_activation(stored_k)
        self.assertTrue(
            torch.allclose(reconstructed.float(), raw_k.float(), atol=1e-2),
            "rotate_activation(stored_key) should equal raw_key — "
            "meaning stored_key had Hadamard applied",
        )
        # Stored key and raw key must differ (Hadamard actually changes the values)
        self.assertFalse(
            torch.allclose(stored_k.float(), raw_k.float(), atol=1e-3),
            "stored key and raw key should differ: Hadamard was supposed to be applied",
        )
        # Stored key must be BF16 (dtype expected by fused_store_index_k_cache)
